...
{
    tree *tmp = (tree *)malloc(sizeof(tree) * 128);//한번 동적 할당
    memset(tmp, 0x0, sizeof(tree) * 128); //tree 128개 만큼의 공간 확보
...
//cf 삭제된 공간은 free_away 를 따로 만들어 그것에 비어진 공간의 주소 또는 index를 저장해서 관리
    m->arr[idx].data = data;
    m->elem_num++; //지워진 공간이 있더라도 이전에  사용한 공간을 알기 위해서
