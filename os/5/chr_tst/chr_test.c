...
int mydrv_init(void)
{
		...
		// dev/ 장치 파일 밑에 "mydeviedfile"이 생성됨
        mydevice = device_create(myclass, NULL, mydev, NULL, "mydevicefile");
        if(IS_ERR(mydevice))
        {
				//에러 발생시 설정한 드라이버의 종류를 해제하고 등록을 해제함
		...
		//kmalloc 이므로 buddy 및 slab을 활용하게 되어있음
...
MODULE_LICENSE("GPL");			//module lincense GPL과 MIT가 있음
